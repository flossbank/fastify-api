...
module.exports = async (req, res, ctx) => {
  try {
     ...
     */
    const { access_token: accessToken } = await ctx.github.requestAccessToken({ code, state })
    const { email } = await ctx.github.requestUserData({ accessToken })
    let user = await ctx.db.user.getByEmail({ email })
    if (!user) user = await ctx.db.user.create({ email })
    await ctx.db.user.attachAccessToken({ userId: user.insertedId || user.id, accessToken })
