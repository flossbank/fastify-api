...
test.before(async (t) => {
  await before(t, async ({ db, auth }) => {
    ...
    const { id: userId4 } = await db.user.createUser({ email: 'rip@vanwinkle.com' })
    t.context.userId4 = userId4.toHexString()
    await db.user.updateUserCustomerId({ userId: t.context.userId4, customerId: 'honesty-cust-id-4' })
    await db.user.updateUserHasCardInfo({ userId: t.context.userId4, last4: '2222' })
...
test('PUT `/user/donation` 200 success | threshold below ad threshold', async (t) => {
  ...
  const user = await t.context.db.user.getUserById({ userId: t.context.userId4 })
