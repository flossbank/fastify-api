...
const login = require('../../../api/advertiser/login')
...
test('reject with invalid password', async (t) => {
  ...
  mockFastify.mongo.collection.returns({
    findOne: sinon.stub().returns(advertiser)
  })
...
test('succesful login', async (t) => {
  sinon.stub(auth, 'createAdvertiserSession').resolves('1234')
  ...
  t.true(mockFastify.mongo.collection.calledWith('advertisers'))
  t.true(mockFastify.mongo.collection().findOne.calledWith({ email: 'pete@pete.com' }))
  t.true(res.setCookie.calledWith(advertiserSessionKey, '1234'))
  mockFastify.mongo.collection.reset()
  auth.createAdvertiserSession.restore()
})
