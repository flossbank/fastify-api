...
test('email | activate user', (t) => {
  ...
  t.true(res.Body.Text.Data.includes(
    'https://verification.flossbank.com/?email=3zvxr&token=bar&kind=user'
  ))
})
...
test('email | activate advertiser', (t) => {
  ...
  t.is(res.Subject.Data, 'Verify your Flossbank email address')
  t.true(res.Body.Text.Data.includes(
    'https://verification.flossbank.com/?email=3zvxr&token=bar&kind=advertiser'
  ))
})
...
test('email | activate maintainer', (t) => {
  ...
  t.is(res.Subject.Data, 'Verify your Flossbank email address')
  t.true(res.Body.Text.Data.includes(
    'https://verification.flossbank.com/?email=3zvxr&token=bar&kind=maintainer'
  ))
})
